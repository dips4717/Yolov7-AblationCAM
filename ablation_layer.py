...
class AblationLayerYOLOv7(AblationLayer):
    def __init__(self):
        super(AblationLayerYOLOv7, self).__init__()

    def __call__(self, x):
        result = self.activations
        subval = [0]*128 + [128]*256 + [384]*512
        quotient2layer_id = {0:0, 1:1, 2:1, 3:2, 4:2, 5:2, 6:2}
        num_channels_to_ablate = result[0].size(0)      # batch_size: 32 
        for i in range(num_channels_to_ablate):
            quotient = int(i/128)
            layer_id = quotient2layer_id[quotient]
            index_in_layer = i - subval[i]
            result[layer_id][i,
                                          index_in_layer, :, :] = 0 # -100000000 # 0# -1000 # -100000000
        return result
    ...
    def set_next_batch(
        ...
        for value in activations:
            yolo_activation_i = value[input_batch_index,
                                :, :, :].clone().unsqueeze(0)
            

            self.activations.append(yolo_activation_i.repeat(num_channels_to_ablate,1,1,1))
