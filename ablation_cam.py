...
class AblationCAM(BaseCAM):
    ...
    def save_activation(self, module, input, output) -> None:
        ...
        if isinstance(output, list):  # Quick Fix YOLO.  This is yolo backbone output, this is an issue if we simply assign output, something to do with mem alloc 
            self.activations = [x.clone() for x in output] #output.clone()
    ...
    def get_cam_weights(self,
        ...
        with torch.no_grad():
            ...
            outputs = self.model(input_tensor)  # For FasterRCNN, list of dicts for boxes, label, score, after NMS
            handle.remove()

            if isinstance(outputs, tuple):   # Yolo returns a tuple of atensor and a list of tensors (torch.Size([1, 10647, 14]), [torch.Size([1, 3, 52, 52, 14]), torch.Size([1, 3, 26, 26, 14]), torch.Size([1, 3, 13, 13, 14])]  )
                outputs = [outputs[0]]  
            
            elif isinstance(outputs, list):  # For Faster-RCNN
                pass
        ...
        number_of_channels = activations.shape[1]  # faster-rcnn shape = [1,1280,13,17] yolo-shape [1,896,13,13]
        weights = []
        ...
        with torch.no_grad():
            ...
            for batch_index, (target, tensor) in enumerate(
                ...
                new_scores = []
                batch_tensor = tensor.repeat(self.batch_size, 1, 1, 1)   #  [32, 3, 416, 416]
        ...
        print('Weights- len, sum :', weights.shape, weights.sum())
